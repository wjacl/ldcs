...
public class LiveDataTJDao {
	...
	String borkerIdsStr = (String) params.get("brokerId_in_String");
	if (StringUtils.isNotBlank(borkerIdsStr)) {
	    sql.append(" and a.broker_id in ('" + borkerIdsStr.replace(",", "','") + "') ");
	}
	...
	for (Object[] os : list) {
	    ...
	    i++;
	    eva.setGiftEarning(os[i] == null ? null : ((BigDecimal) os[i]).doubleValue());
	    i++;
	    eva.setLiveDuration(os[i] == null ? null : ((BigDecimal) os[i]).intValue());
	    i++;
	    eva.setGiftEarningGoal(os[i] == null ? null : ((BigDecimal) os[i]).doubleValue());
	    i++;
