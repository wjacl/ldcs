    ...
    public Page<EvaLiverMonth> tongJiPageQuery(Map<String, Object> params, Page<EvaLiverMonth> page)
    {
        ...
        if (StringUtils.isNotBlank(sort))
        {
            ...
            for (int i = 0; i < ss.length; i++)
            {
                if (i == 0)
                {
                    ...
                    {
                        sql.append(" order by " + ss[i] + " " + (oss[i] == null ? "" : oss[i]));
                    }
                }
                else
                {
                    ...
                    {
                        sql.append(" ," + ss[i] + " " + (oss[i] == null ? "" : oss[i]));
                    }
                }
            }
        }
        ...
        if (total >= page.getStartNum())
        {
            ...
            for (Object[] os : list)
            {
                ...
                eva.setMonth(Integer.valueOf((String)os[i++]));
                eva.setGiftEarning(new BigDecimal((Double)os[i++]));
                eva.setLiveDuration((Integer)os[i++]);
                eva.setGiftEarningGoal(new BigDecimal((Double)os[i++]));
