...
public class PrivilegeControlService {
	...
	if (user != null) {
		...
		// 领导用户，可以查看所在部门及子部门下的经纪人下的主播
		this.orgService.getAllChildOrg(user.getOrg(), orgs, null, null);
		this.addBrokerIdCondition(orgs, params);

	    } else if (User.TYPE_NORMAL_STAFF.equals(user.getType())) {
		// 普通员工用户，只可查看所在部门的经纪人下的主播数据
		this.addBrokerIdCondition(orgs, params);
	    } else {
		// 经纪人，只可查看所负责的主播
		params.put("brokerId_eq_String", user.getId());
	    }
	}
    }

    private void addBrokerIdCondition(List<Org> orgs, Map<String, Object> params) {
	...
	List<User> us = this.userService.query(p);
